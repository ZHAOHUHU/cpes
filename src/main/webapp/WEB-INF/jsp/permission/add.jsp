        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
			...
			<div class="panel panel-default">
			  ...
			  <div class="panel-body">
				<form id="permissionForm" role="form" action="${APP_PATH}/permission/insert.do" method="post">
				  <input type="hidden" name="pid" value="${param.pid}">
				  <div class="form-group">
					<label for="exampleInputPassword1">许可名称</label>
					<input type="text" class="form-control" id="fname" name="name" placeholder="许可名称">
				  </div>
				  <div class="form-group">
					<label for="exampleInputPassword1">请求路径</label>
					<input type="text" class="form-control" id="furl" name="url" placeholder="请求路径">
				  </div>
        ...
        <script type="text/javascript">
            $(function () {
			    ...
			    $("#insertBtn").click(function(){
			    	...
			    	var furl = $("#furl");
			    	if ( furl.val() == "" ) {
			    		layer.msg("请求路径不能为空，请输入", {time:2000, icon:5, shift:6}, function(){
			    			furl.focus();
			    		});
			    		return;
			    	}
			    	...
			    	*/
			    	$("#permissionForm").ajaxSubmit({
			    		...
			    		success : function(r) {
			    			if ( r.success ) {
				    			layer.confirm("许可信息保存成功,是否连续输入",  {icon: 3, title:'提示'}, function(cindex){
				    				layer.close(cindex);
				    			}, function(cindex){
				    				window.location.href = "${APP_PATH}/permission/tree.htm";
				    			    layer.close(cindex);
				    			});
			    			} else {
					    		layer.msg("许可信息保存失败", {time:2000, icon:5, shift:6});
			    			}
			    		}
			    	});
