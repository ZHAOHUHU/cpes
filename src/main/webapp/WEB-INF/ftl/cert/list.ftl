 ...
 <hr style="clear:both;">
    ...
    <script type="text/javascript">
        ...
        function queryCert(pageindex) {
        	...
        	// 使用AJAX异步查询资质信息
        	$.ajax({
        		url : "${APP_PATH}/cert/queryPageCerts.do",
        		type : "POST",
        		data : paramObj,
        		beforeSend : function() {
        			...
        			if ( r.success ) {
        				...
        				$.each(datas, function(index, cert){
        	                ...
        	                content += '<tr>';
	      	                ...
	      	                content += '  <td>';
	      					content += '      <button type="button" class="btn btn-success btn-xs" onclick="window.location.href=\'${APP_PATH}/cert/assign.htm?id='+cert.id+'\'"><i class="glyphicon glyphicon-check"></i></button>';
        				...
        				$("#Pagination").pagination(page.totalsize, {
        					...
        					next_text:"下一页",
        					ellipse_text:"...",
        					callback: queryCert, // 当点击页码链接时，会执行回调方法，执行查询操作
        ...
        function deleteCert(id, name) {
			layer.confirm("删除资质["+name+"]信息,是否继续？",  {icon: 3, title:'提示'}, function(cindex){
				...
				$.ajax({
					...
					success : function(r) {
						if ( r.success ) {
							layer.msg("资质["+name+"]信息删除成功", {time:2000, icon:6}, function(){
								queryCert(0);
							});
						}
					}
