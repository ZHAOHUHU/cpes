 ...
 <div class="navbar-wrapper">
    ...
    <div class="container theme-showcase" role="main">
      <div class="page-header">
		...
		<form id="applyForm" method="post" enctype="multipart/form-data" action="${APP_PATH}/member/doapply4.do" role="form" style="margin-top:20px;">
			  <div class="form-group">
				<label for="frealname">认证码</label>
				<input type="text" class="form-control" id="fcertno" name="certno" placeholder="Enter email">
			  </div>
		  <button id="applyBtn" class="btn btn-success">继续提交</button>
		</form>
		<hr>
	...
	<script>
        ...
        $("#applyBtn").click(function(){
	    	...
	    	$("#applyForm").ajaxSubmit({
	    		...
	    		success : function(r) {
	    			if ( r.success ) {
		    			layer.msg("会员实名认证申请成功，请等待审批结果", {time:2000, icon:6});
	    			} else {
